export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-news-ai', region: config.region, category: config.category } });

  const res = await fetch(`${baseUrl}/tools/news-ai`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`NewsAI ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
