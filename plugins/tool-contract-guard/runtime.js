export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-contract-guard' } });

  const res = await fetch(`${baseUrl}/tools/contract-guard`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
    body: JSON.stringify({ input: stringifyMsg(msg) }),
  });

  if (!res.ok) {
    const t = await res.text().catch(() => '');
    throw new Error(`ContractGuard ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
