export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-postiz', platforms: config.platforms } });

  const res = await fetch(`${baseUrl}/tools/postiz`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`Postiz ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
