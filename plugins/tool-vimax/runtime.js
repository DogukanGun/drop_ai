export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-vimax', episodeCount: config.episodeCount } });

  const res = await fetch(`${baseUrl}/tools/vimax`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
    body: JSON.stringify({
      input: stringifyMsg(msg),
      episodeCount: Number(config.episodeCount ?? 1),
  ...
  if (!res.ok) {
    ...
    throw new Error(`ViMax ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
