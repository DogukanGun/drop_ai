export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-nutrient', operation: config.operation } });

  const res = await fetch(`${baseUrl}/tools/nutrient`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
    body: JSON.stringify({
      input: stringifyMsg(msg),
      operation: config.operation || 'ocr',
  ...
  if (!res.ok) {
    ...
    throw new Error(`Nutrient ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
