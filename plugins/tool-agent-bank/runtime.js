export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-agent-bank', operation: config.operation } });

  const res = await fetch(`${baseUrl}/tools/agent-bank`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`AgentBank ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
