export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-agentic-mail', to: config.to } });

  const res = await fetch(`${baseUrl}/tools/agentic-mail`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`AgenticMail ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
