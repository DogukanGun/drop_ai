export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-pdf-builder', documentType: config.documentType } });

  const res = await fetch(`${baseUrl}/tools/pdf-builder`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
    body: JSON.stringify({
      input: stringifyMsg(msg),
      documentType: config.documentType || 'NDA',
    }),
  });

  if (!res.ok) {
    const t = await res.text().catch(() => '');
    throw new Error(`PDF Builder ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
