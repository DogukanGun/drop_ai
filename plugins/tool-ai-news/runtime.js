export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-ai-news' } });

  const res = await fetch(`${baseUrl}/tools/ai-news`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
    body: JSON.stringify({
      input: stringifyMsg(msg),
      topic: config.topic || '',
      maxArticles: Number(config.maxArticles ?? 5),
    }),
  });

  if (!res.ok) {
    const t = await res.text().catch(() => '');
    throw new Error(`AI News Skill ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
