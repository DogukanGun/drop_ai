export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-pdf-signer' } });

  const res = await fetch(`${baseUrl}/tools/pdf-signer`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
    body: JSON.stringify({
      input: stringifyMsg(msg),
      documentUrl: config.documentUrl,
      signatureImageUrl: config.signatureImageUrl,
    }),
  });

  if (!res.ok) {
    const t = await res.text().catch(() => '');
    throw new Error(`PDF Signer ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
