export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-patent-ai' } });

  const res = await fetch(`${baseUrl}/tools/patent-ai`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`PatentAI ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
