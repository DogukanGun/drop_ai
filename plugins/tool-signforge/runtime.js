export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-signforge' } });

  const res = await fetch(`${baseUrl}/tools/signforge`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`SignForge ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
