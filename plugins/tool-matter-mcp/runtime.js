export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-matter-mcp', deviceId: config.deviceId } });

  const res = await fetch(`${baseUrl}/tools/matter-mcp`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`Matter MCP ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
