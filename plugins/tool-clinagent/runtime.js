export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-clinagent' } });

  const res = await fetch(`${baseUrl}/tools/clinagent`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`ClinAgent ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
