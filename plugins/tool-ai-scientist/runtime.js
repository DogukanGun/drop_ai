export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-ai-scientist', domain: config.domain } });

  const res = await fetch(`${baseUrl}/tools/ai-scientist`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
    body: JSON.stringify({
      input: stringifyMsg(msg),
      domain: config.domain || '',
      maxIterations: Number(config.maxIterations ?? 3),
    }),
  });

  if (!res.ok) {
    const t = await res.text().catch(() => '');
    throw new Error(`AI Scientist ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
