export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-jobclaw', role: config.role, mode: config.mode } });

  const res = await fetch(`${baseUrl}/tools/jobclaw`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
    body: JSON.stringify({
      input: stringifyMsg(msg),
      role: config.role || '',
      mode: config.mode || 'seeker',
    }),
  });

  if (!res.ok) {
    const t = await res.text().catch(() => '');
    throw new Error(`JobClaw ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
