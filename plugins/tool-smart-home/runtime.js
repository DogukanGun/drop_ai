export async function run(ctx, config, msg) {
  ...
  ctx.emit({ kind: 'start', payload: { tool: 'tool-smart-home' } });

  const res = await fetch(`${baseUrl}/tools/smart-home`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`Smart Home ${res.status}: ${t.slice(0, 300) || res.statusText}`);
  }
